...
void amg88xx_sleep(void){
  //enter sleep mode
	...
	write8(AMG88xx_PCTL, getPCTL());
        HAL_Delay(100);                 // обязательна задержка, без неё не засыпал
}

void amg88xx_wake_up(void){
          // normal mode
          _pctl.PCTL = AMG88xx_NORMAL_MODE;
          write8(AMG88xx_PCTL, getPCTL());
          HAL_Delay(50);                 // Time to enable communication after setup
          //initial reset
          ...
          write8(AMG88xx_RST, getRST());
          HAL_Delay(3);
        // все регистры в исходном состоянии, необходимо внести параметры
...
{
	int32_t highConv = (int32_t)(high / AMG88xx_PIXEL_TEMP_CONVERSION);
	...
	int32_t lowConv = (int32_t)(low / AMG88xx_PIXEL_TEMP_CONVERSION);
	...
	int32_t hysConv = (int32_t)(hysteresis / AMG88xx_PIXEL_TEMP_CONVERSION);
